...
logging.basicConfig(format=FORMAT,filename="./log/fps=17/test-B=5e-06-miou.log".format(int(fps),B,fps),filemode="w")
...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        if prev_gray is not None :
            if  iter_fps<(upload_fps)<(iter_fps+(1/17.0)) or iter%17==0:
                last_pred = warp_flow(next_pred, flow_var) # 对segment results应用光流传播到下一帧
                upload_fps+=(1.0/fps)
                type="P"
                prev_gray = cur_gray
                Offloading=True
                next_pred = load_label('/media/xxd/新加卷/Projects/DRN/save/pred_img/B={0}'.format(B) + '/' + name[0].split("/")[-1])
                acc = accuracy_np(last_pred, label[0].cpu().data.numpy())
                ...
                Offloading=False
                prev_gray = cur_gray
                last_pred = warp_flow(last_pred, flow_var) # 对segment results应用光流传播到下一帧
                next_pred=warp_flow(next_pred, flow_var) # 对segment results应用光流传播到下一帧
                #elif not Offloading : pred= warp_flow(pred, flow_var)
                acc = accuracy_np(last_pred, label[0].cpu().data.numpy())
        ...
        if prev_gray is None :# or p < PSNR_B: # 第一帧或psnr小于 门限（即需要上载的帧的处理）
            pred =  load_label('/media/xxd/新加卷/Projects/DRN/save/pred_img/B={0}'.format(B) + '/' + name[0].split("/")[-1])
            last_pred=pred
            next_pred=pred
            prev_gray=cur_gray
        ...
        if iter>0 and iter%17==0:
            print("{0}/{1}:miou-{2}".format(iter,599,100*np.nanmean(per_class_iu(hist))))
            logger.info("{0}/{1}:miou-{2}".format(iter,599,100*np.nanmean(per_class_iu(hist))))
        end = time.time()
    ...
    print("ave acc:{0}".format(all_acc/599))
    ...
    if has_gt: #val 有ground truth
        ...
        for id_class in range(num_classes):
            print("==>{0}:      mIou:  {1}".format(id_class,(round(ious[id_class],2))))
            logger.info("==>{0}:      mIou:  {1}".format(id_class,(round(ious[id_class],2))))
        print("mIou:{0}".format(round(np.nanmean(ious),2)))
        logger.info("mIou:{0}".format(round(np.nanmean(ious),2)))
