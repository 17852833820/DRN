...
logging.basicConfig(format=FORMAT,filename="./log/H264/gop=34_QP=1.log",filemode="w")
...
class SegList(torch.utils.data.Dataset):
    ...
    def read_lists(self):
        image_path = join(self.list_dir,   'gop=34_images_qp=1_path.txt') #读取图像路径列表list'./datasets/testimg/test_images.txt'
...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        I_number+=1
        Offloading=True
        image = jpeg.normalize(cur_img) # 正则化
        """上载"""
        ...
        """log"""
        logger.info("Frame id :{0}, Row size:{1} KB, Type: {2}, Offloading:{3}, compressed size:{4}".format(iter+1,row_size,type,Offloading,row_size))
        import numpy as np
