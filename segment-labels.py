...
from src import drn
from src import data_transforms as transforms
...
class SegListMS(torch.utils.data.Dataset):
    ...
    def read_lists(self):
        image_path = join(self.list_dir, "demovideo", 'image_path.txt') #读取图像路径列表list'./datasets/testimg/test_images.txt'
        #image_path = join(self.list_dir, self.phase + '_images.txt')
        #label_path = join(self.list_dir, self.phase + '_labels.txt')
        assert exists(image_path)
        self.image_list = [line.strip() for line in open(image_path, 'r')]
...
def save_output_images(predictions, filenames, output_dir):
    ...
    for ind in range(len(filenames)):
        ...
        fn = os.path.join(output_dir, filenames[ind].split("/")[-1][:-4] + '.png')
        out_dir = split(fn)[0]
...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        batch_time.update(time.time() - end)  
        if save_vis:
            save_output_images(pred, name, output_dir='./datasets/back_test/test_labels')
            save_colorful_images(
                pred, name, './datasets/back_test/test_labels_color',
                TRIPLET_PALETTE if num_classes == 3 else CITYSCAPE_PALETTE)
            print("save fig {}".format(iter))
        end = time.time()
...
def test_seg(args):
    ...
    data_dir = args.data_dir
    info = json.load(open('./datasets/testimg/info.json', 'r'))
...
def parse_args():
    ...
    parser.add_argument('-cmd', default="test")
    parser.add_argument('-d', '--data-dir', default="./datasets/back_test")
    ...
    parser.add_argument('--pretrained', dest='pretrained',
                        default='./pretrained/drn_d_22_cityscapes.pth', type=str, metavar='PATH',
