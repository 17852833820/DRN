    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        if prev_gray is not None :
            if  iter_fps<(upload_fps)<(iter_fps+(1/17.0)) or iter%17==0 or fps==17:
                ...
                # save_output_images(pred, name, output_dir='/media/xxd/新加卷/Projects/DRN/save-fps-drn22/fps={0}/pred_img/B={1}-fps={2}-95.41-full'.format(int(fps),B,fps))
                # pred_jpeg = load_label('/media/xxd/新加卷/Projects/DRN/save-fps-drn22/fps={0}/pred_img/B={1}-fps={2}-95.41-full'.format(int(fps),B,fps) + '/' + name[0].split("/")[-1])
                ...
                # upload_num += 1
                # save_output_images(pred, name, output_dir='/media/xxd/新加卷/Projects/DRN/save-fps-drn22/fps={0}/pred_img/B={1}-fps={2}-95.41-full'.format(int(fps),B,fps))
                pred = load_label('/media/xxd/新加卷/Projects/DRN/save-fps-drn22/fps={0}/pred_img/B={1}-fps={2}'.format(int(fps),B,fps) + '/' + name[0].split("/")[-1])
                ...
                save_output_images(pred, name, output_dir='/media/xxd/新加卷/Projects/DRN/save-fps-drn22/fps={0}/pred_img/B={1}-fps={2}'.format(int(fps),B,fps))
                pred = load_label('/media/xxd/新加卷/Projects/DRN/save-fps-drn22/fps={0}/pred_img/B={1}-fps={2}'.format(int(fps),B,fps) + '/' + name[0].split("/")[-1])
                acc = accuracy_np(pred, label[0].cpu().data.numpy())
                sum_acc+=acc
                sum_size+=size
        ...
        if prev_gray is None :# or p < PSNR_B: # 第一帧或psnr小于 门限（即需要上载的帧的处理）
            ...
            # #save_encode_images(rec_image.clone().cpu().data.numpy(),name,output_dir='/media/xxd/新加卷/Projects/DRN/save-back-fps/fps={0}/encode_img/B={1}-fps={2}'.format(int(fps),B,fps))
