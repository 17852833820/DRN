...
logging.basicConfig(format=FORMAT,filename="./log/baseline2/fps={0}-qp={1}.log".format(int(fps-1),qp),filemode="w")
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        if prev_gray is not None :
            if  iter_fps<(upload_fps)<(iter_fps+(1/17.0)) or iter%17==0 or fps==17.0:
                ...
                save_output_images(pred, name, output_dir='/media/xxd/新加卷/Projects/DRN/save-fps-drn22/baseline2/fps={0}/pred_img/fps={1}-qp={2}'.format(int(fps),fps,qp))
                ...
                pred = load_label('/media/xxd/新加卷/Projects/DRN/save-fps-drn22/baseline2/fps={0}/pred_img/fps={1}-qp={2}'.format(int(fps),fps,qp) + '/' + name[0].split("/")[-1])
