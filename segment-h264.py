...
logging.basicConfig(format=FORMAT,filename="./log/H264/h264-qp={0}_copy.log".format(qp),filemode="w")
...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        if prev_gray is not None :#p < PSNR_B:# 非第一帧的处理
            ...
            sum_size += size
            upload_num += 1
            save_output_images(pred, name, output_dir='/media/xxd/新加卷/Projects/DRN/save-fps-drn22/H264/qp={0}'.format(qp))
            pred = load_label('/media/xxd/新加卷/Projects/DRN/save-fps-drn22/H264/qp={0}'.format(qp)+ '/' + name[0].split("/")[-1])
        ...
        if prev_gray is None :# or p < PSNR_B: # 第一帧或psnr小于 门限（即需要上载的帧的处理）
            ...
            # save_encode_images(rec_image.clone().cpu().data.numpy(),name,output_dir='/media/xxd/新加卷/Projects/DRN/save-back-fps/test/encode_img/B={0}'.format(B))
            ...
            upload_num += 1
            save_output_images(pred, name, output_dir='/media/xxd/新加卷/Projects/DRN/save-fps-drn22/H264/qp={0}'.format(qp))
            #save_colorful_images(pred, name, '/media/xxd/新加卷/Projects/DRN/save-back-fps/pred_img' + '_color/B={0}'.format(B),TRIPLET_PALETTE if num_classes == 3 else CITYSCAPE_PALETTE)
            pred = load_label('/media/xxd/新加卷/Projects/DRN/save-fps-drn22/H264/qp={0}'.format(qp) + '/' + name[0].split("/")[-1])
            acc = accuracy_np(pred, label[0].cpu().data.numpy())
            accuracys.append(acc)
            sum_acc += acc
