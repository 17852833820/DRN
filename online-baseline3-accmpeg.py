...
logging.basicConfig(format=FORMAT,filename="./run_log/youtube_drn38_4gLTE_baseline3.log",filemode="w")
...
class SegList(torch.utils.data.Dataset):
    ...
    def read_lists(self):
        image_path = join(self.list_dir,    "demovideo", 'back_test_images_path.txt') #读取图像路径列表list'./datasets/testimg/test_images.txt'
        label_path = join(self.list_dir,  "demovideo", 'back_test_labels_path.txt')#读取语义分割label真实标签路径列表list'./datasets/testimg/test_labels.txt'
...
def test(eval_data_loader, model, num_classes,
    ...
    search_tables=pd.read_hdf("./youtube_drn22_log/table_youtube_drn22_baseline3.hdf5",key="df",mode="r")
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        flow_var = np.load('/media/xxd/新加卷/Projects/DRN/DIS/back_test_flow/' + filename.replace(".png", '.npy')).astype(np.float32)    # 读取filename对应名字的光流
        ...
        if prev_gray is not None :
           if  iter_fps<(upload_fps)<(iter_fps+(1/17.0)) or iter%17==0 or fps==17:
                ...
                #3. 对残差进行jpeg压缩
                selected_tables2 = selector.extract_table2(table_indexs1) # 通过传播后的索引提取量化表
                ...
                #save_encode_images(rec_image.clone().cpu().data.numpy(),name,output_dir='/media/xxd/新加卷/Projects/DRN/save-fps-drn22/baseline3/fps={0}/encode_img/B={1}-fps={2}'.format(int(fps),B,fps))
                ...
                upload_num += 1
            #    if  fps==17:
                save_output_images(pred, name, output_dir='/media/xxd/新加卷/Projects/DRN/run_log/pred_img/youtube_drn38_4gLTE/baseline3/')
                ...
                pred = load_label('/media/xxd/新加卷/Projects/DRN/run_log/pred_img/youtube_drn38_4gLTE/baseline3/'+ '/' + name[0].split("/")[-1])
            #    else:
            #        pred.resize(1024,2048)
                ...
                g = dct_var.grad.clone() # 得到关于残差dct系数的梯度g
                #pred = pred.cpu().data.numpy()
                table_indexs1 = selector.choose_table2_baseline3(s, g,qp) # 根据dct系数和grad选择量化表
           else:
                type="P"
                Offloading=False
                size=0
                ...
                save_output_images(pred, name, output_dir='/media/xxd/新加卷/Projects/DRN/run_log/pred_img/youtube_drn38_4gLTE/baseline3/')
                preda = load_label('/media/xxd/新加卷/Projects/DRN/run_log/pred_img/youtube_drn38_4gLTE/baseline3/'+ '/' + name[0].split("/")[-1])
