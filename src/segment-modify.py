...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        # torch.save(dct_var, f'./pth/dct{num}.pth')
        dct_var.requires_grad = True
        image_var = jpeg.idct(dct_var)
        # image_var = jpeg.normalize(image_var)
        # print(image_var)
        final = model(image_var)[0]
        # print(accuracy(final.cpu().data, labels.cpu().data))

        _, pred = torch.max(final, 1)
        # print(accuracy(final.data, pred.data))
        # loss = criterion(final, pred.data)
        loss = criterion(final, pred.data)
        print(loss)
        # print(criterion(final, labels.cuda().data))
        loss.backward()
        # torch.save(dct_var.grad, f'./pth/grad{num}.pth')
        # print(torch.sum(dct_var.grad*dct_var))
        # print(dct_var.grad)
        # from torch.autograd import grad
        # loss_grads = grad(loss, dct_var, create_graph=True)
        num += 1
        input()
        model.zero_grad()
