...
def test(eval_data_loader, model, num_classes,
    ...
    criterion = nn.NLLLoss2d(ignore_index=255)
    criterion.cuda()
    # qtables = torch.load('./pth/plain_q_tables.pth')
    qtables = torch.load('./pth/jpeg_qtables.pth')
    num = 0
    N = 25
    # acc = torch.zeros(N)
    # ll = torch.zeros(N)
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        # q = qtables[92-1].reshape(1, 3, 1, 8, 8)
        prev_gray = None
        glitch = None
        prev_var = None
        flow_var = np.load('./DIS/flow/' + filename.replace(f'_leftImg8bit.png', '.npy')).astype(np.float32)
        for index, i in enumerate(range(n-19, n+11)):
            # print(index, i)
            cur_img = f'datasets/testimg/leftImg8bit_sequence/val/{dirname}/' + filename.replace(f'{n:06d}_leftImg8bit.png', f'{i:06d}_leftImg8bit.png')
            ...
            if prev_gray is not None:
                ...
                # print('propogation', p)
            if  prev_gray is None or p < PSNR_B:
                prev_gray = cur_gray
                glitch = prev_gray.copy()
                ToPILImage()(cur_img.squeeze(0)).save('tmp/jpeg_test.jpg', quality=90)
                compressed = to_tensor(Image.open('tmp/jpeg_test.jpg')).unsqueeze(0)
                prev_var = (index, compressed)
                size = os.path.getsize('tmp/jpeg_test.jpg')/1024
                # print('size: ', size)
                sum_size += size
                upload_num += 1

            if i == n:
                image = jpeg.normalize(prev_var[1])
                final = model(image)[0]
                _, pred = torch.max(final, 1)
                pred = pred.cpu().data.numpy()
                save_output_images(pred, name, output_dir)
                pred = load_label(output_dir + name[0])
                print(prev_var[0], ' to ', '19')
