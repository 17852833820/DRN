...
class RandomCrop(object):
    ...
    def __call__(self, image, label, *args):
        assert label is None or image.size == label.size, \
            "image and labels doesn't have the same size {} / {}".format(
...
class ToTensor(object):
    ...
    def __call__(self, pic, label=None):
        if isinstance(pic, np.ndarray):
            # handle numpy array：numpy to tensor
            img = torch.from_numpy(pic)
        else:
            # handle PIL Image：PIL to tensor
        ...
        img = img.float().div(255)#图像像素从255归一化到0-1
        if label is None:
            return img,
        else:
            return img, torch.LongTensor(np.array(label))
