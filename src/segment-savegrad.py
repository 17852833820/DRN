...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        ToPILImage()(image.squeeze(0)).save('tmp/jpeg_test.jpg', quality=100)
        image = to_tensor(Image.open('tmp/jpeg_test.jpg')).unsqueeze(0)

        dct_var = jpeg.dct(image)

        dct_var.requires_grad = True
        image = jpeg.idct(dct_var)
        im = jpeg.normalize(image)
        final = model(im)[0]
        _, pred = torch.max(final, 1)
        loss = criterion(final, label.cuda().data)
        loss.backward()
        g = dct_var.grad.clone()
        torch.save(g, "./pth/grad1-jpeg100.pth")
        print(num, accuracy(final.data, label.cuda().data))
        sum_acc += accuracy(final.data, label.cuda().data)
        # size = jpeg.torchjpeg_compress(dct_int_var.clone())
        # print(size)
        # sum_size += size
