...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        ## self
        def choose_table(s, g, B):
            candidate = torch.load('./pth/plain_q_tables.pth')
            tables = candidate[15:25]
            tables = torch.zeros(3, 3, 8, 8)
            tables[0] = candidate[15]
            tables[1] = candidate[20]
            tables[2] = candidate[25]
            # tables[3] = candidate[25]
            s = blockify(s, 8) 
            g = blockify(g, 8) 
            selected_tables = torch.zeros(1, 3, s.shape[2], 8,  8)
            for i in range(3):
                for j in range(s.shape[2]):
                    e = s[0, i, j] - tables[:, i] * torch.round(s[0, i, j] / tables[:, i])
                    # e = tables[:, i]/2
                    e *= g[0, i, j]
                    e = torch.sum(torch.abs(e), dim=[1, 2])
                    selected_tables[0, i, j] = tables[torch.argmin(torch.abs(e - B)), i]
            return selected_tables
        dct_var = jpeg.dct(image.clone())
        q = qtables[25].reshape(1, 3, 1, 8, 8)
        dct_int_var = jpeg.compress(image.clone(), q)
        dct_var = jpeg.dequantize(dct_int_var, q, normalize=False)
        s = dct_var.clone()
        dct_var.requires_grad = True
        im = jpeg.idct(dct_var)
        im = jpeg.normalize(im)
        final = model(im)[0]
        _, pred = torch.max(final, 1)
        loss = criterion(final, label.cuda().data)
        loss.backward()
        g = dct_var.grad.clone()
        # torch.save(g, './pth/grad0-select.pth')
        selected_tables = choose_table(s, g, 0.9e-4)
        dct_var = jpeg.compress(image, selected_tables)
        image = jpeg.decompress(dct_var, selected_tables, normalize=False)
        image = jpeg.normalize(image)
        final = model(image)[0]
        _, pred = torch.max(final, 1)
        print(num, accuracy(final.data, label.cuda().data))
        sum_acc += accuracy(final.data, label.cuda().data)
        size = jpeg.torchjpeg_compress(dct_var.clone())
        print(size)
        sum_size += size
