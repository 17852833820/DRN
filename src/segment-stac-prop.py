...
class SegList(torch.utils.data.Dataset):
    ...
    def read_lists(self):
        image_path = join(self.list_dir, self.phase + '_images.txt') #读取图像路径列表list'./datasets/testimg/test_images.txt'
        label_path = join(self.list_dir, self.phase + '_labels.txt')#读取语义分割label真实标签路径列表list'./datasets/testimg/test_labels.txt'
...
def train_seg(args):
    ...
    """optionally resume恢复 from a checkpoint"""
    if args.resume:#可选择从哪个保存点恢复
...
def test(eval_data_loader, model, num_classes,
    ...
    selector = Selector(B=2.7e-4, region=2)#初始化，B
    PSNR_B = 26
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    from DIS import Flow, accuracy_np, load_label, psnr, warp_flow
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        pattern = re.compile(r'(.*?)_\d{6}_(\d{6})_leftImg8bit\.png')
        dirname, n = pattern.findall(filename)[0]
        n = int(n)
        print(dirname, n, num)

        qtables = torch.load('./pth/jpeg_qtables.pth')      #读取jpeg量化表
        prev_gray = None
        glitch = None
        prev_var = None
        flow_var = np.load('./DIS/flow/' + filename.replace(f'_leftImg8bit.png', '.npy')).astype(np.float32)    # 读取光流
        for index, i in enumerate(range(n-19, n+11)): # 共有30帧，第20帧有GT真实值
            # print(index, i)
            cur_img = f'datasets/testimg/leftImg8bit_sequence/val/{dirname}/' + filename.replace(f'{n:06d}_leftImg8bit.png', f'{i:06d}_leftImg8bit.png') # 读取第index帧
            cur_img = transforms.ToTensor()(Image.open(cur_img))[0].unsqueeze(0) # 转换为tensor
            cur_gray = Flow().tensor2gray(cur_img) # 转灰度图
            if prev_gray is not None: # 非第一帧的处理
                glitch = warp_flow(glitch, flow_var[index-1]) # 对帧应用光流传播到下一帧
                table_indexs = warp_flow(table_indexs, flow_var[index-1]) # 对量化表索引进行传播
                # q = table_indexs
                p = psnr(glitch, cur_gray) # 计算多次传播后的帧与真实帧的PSNR
                # print('propogation', p)

            if prev_gray is None or p < PSNR_B: # 第一帧或者PSNR低于门限
                if prev_gray is not None: # 如果不是第一帧的话
                    selected_tables = selector.extract_table(table_indexs) # 通过传播后的索引提取量化表
                    q = selected_tables.clone() 
                else:
                    q = qtables[85 - 1].reshape(1, 3, 1, 8, 8) # 第一帧使用jpeg（Q=85）量化表
                prev_gray = cur_gray
                glitch = prev_gray.copy()

                """    改为使用H264指定量化级别压缩当前帧,反量化      """
                compressed = jpeg.compress(cur_img, q) # 指定 QP使用torchjpeg压缩当前帧
                dct_var = jpeg.dequantize(compressed, q, normalize=False) # 反量化得到dct系数s
                ...
                """改为使用H264 反DCT变换"""
                image = jpeg.idct(dct_var) # 逆dct
                image = jpeg.normalize(image) # 归一化


                final = model(image)[0] # 输入到网络计算
                _, pred = torch.max(final, 1) 
                loss = criterion(final, pred.cuda().data)
                loss.backward()
                g = dct_var.grad.clone() # 得到关于dct系数的梯度g
                pred = pred.cpu().data.numpy()

                table_indexs = selector.choose_table(s, g) # 根据dct系数和grad选择量化表
                prev_var = (index, pred)
            ...
            if i == n:
                ...
                pred = load_label(output_dir + '/' + name[0])
                print(index, ' to ', '19')
                for k in range(prev_var[0], 19):
                    pred = warp_flow(pred, flow_var[k])
                    # cv.imshow('pred', pred)
                    # cv.destroyAllWindows()

                acc = accuracy_np(pred, label.cpu().data.numpy())
                print('acc: ', acc)
                sum_acc += acc
                result_pred = pred
        num += 1
        ...
        if has_gt:
            label = label.numpy()
            hist += fast_hist(result_pred.flatten(), label.flatten(), num_classes)
    ...
    print('average sum size: ', sum_size/num/30)
    print('average upload size: ', sum_size/upload_num)
    print('sum acc: ', sum_acc/num)
    print('upload img num:', upload_num)
    print('bias', selector.calc_bais())
    print('average sum size with bias', (sum_size + upload_num * selector.calc_bais())/num/30)
...
def test_seg(args):
    ...
    out_dir = '{}_{:03d}_{}'.format(args.arch, start_epoch, phase)#输出文件夹'drn_d_22_000_test'
