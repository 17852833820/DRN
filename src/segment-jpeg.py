...
def test(eval_data_loader, model, num_classes,
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        ToPILImage()(image.squeeze(0)).save('tmp/jpeg_test.jpg', quality=95)
        image = to_tensor(Image.open('tmp/jpeg_test.jpg')).unsqueeze(0)
        # dct_var = jpeg.dct(image.clone())
        # q = qtables[95 - 1].reshape(1, 3, 1, 8, 8)
        # dct_var = jpeg.dct(image.clone())
        # compressed = jpeg.compress(image, q)
        # image = jpeg.decompress(compressed, q, normalize=False)
