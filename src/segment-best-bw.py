...
def test(eval_data_loader, model, num_classes,
    ...
    selector = Selector(B=2.7e-4, region=2)
    PSNR_B = 26
    model.eval()
    batch_time = AverageMeter()
    data_time = AverageMeter()
    ...
    import re
    from DIS import Flow, warp_flow, psnr, accuracy_np, load_label
    import cv2 as cv
    import pandas as pd
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        for index, i in enumerate(range(n-19, n+11)): # 共有30帧，第20帧有GT
            ...
            cur_label = load_label('datasets/labels/drn_d_38_000_test/' + name[0].replace('leftImg8bit/', 'leftImg8bit_sequence/'))
            cur_img = transforms.ToTensor()(Image.open(cur_img))[0].unsqueeze(0)
            cur_gray = Flow().tensor2gray(cur_img)

            # 第一帧 保证不跨帧获得结果 index == 0
            if index == 0:# prev_gray is None:
                q = qtables[85 - 1].reshape(1, 3, 1, 8, 8)
                prev_gray = cur_gray
                glitch = prev_gray.copy()

                compressed = jpeg.compress(cur_img, q)
                dct_var = jpeg.dequantize(compressed, q, normalize=False)
                s = dct_var.clone()
                dct_var.requires_grad = True

                image = jpeg.idct(dct_var)
                image = jpeg.normalize(image)
                final = model(image)[0]
                _, pred = torch.max(final, 1)
                loss = criterion(final, pred.cuda().data)
                loss.backward()
                g = dct_var.grad.clone()
                ...
                file_constrain = 280
                size = 0
                while True: # 使用二分法计算最佳的B
                    B_mean = (B_min + B_max)/2
                    selector = Selector(B=B_mean, region=2)
                    tables = selector.extract_table(selector.choose_table(s, g)).clone()
                    compressed = jpeg.compress(cur_img, tables)
                    size = jpeg.torchjpeg_compress(compressed.clone())
                    if file_constrain - size > 0.1:
                        print(B_mean, size)
                        break       
                input()
                continue
            ...
            #     glitch = warp_flow(glitch, flow_var[index-1 ])
            #     table_indexs = warp_flow(table_indexs, flow_var[index-1])


            #     while
            #     # q = table_indexs 
            #     p = psnr(glitch, cur_gray)
            #     # print('propogation', p)

            # if p < PSNR_B:
            #     selected_tables = selector.extract_table(table_indexs)
            ...
            if i == n:
                ...
                pred = load_label('datasets/labels/drn_d_38_000_test/' + name[0].replace('leftImg8bit/', 'leftImg8bit_sequence/'))
                print(index, ' to ', '19')
