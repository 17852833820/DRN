    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        def choose_table(s, g, B):
            ...
            for i in range(3):
                ...
                select_indexs = torch.argmin(torch.abs(e - B), dim=0)
                # selected_tables[0, i] = tables[select_indexs, i]
            return select_indexs

        dct_var = jpeg.dct(image.clone())
        q = qtables[94].reshape(1, 3, 1, 8, 8)
        ...
        selected_tables = choose_table(s, g, 1.5e-4)
