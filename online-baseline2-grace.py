...
logging.basicConfig(format=FORMAT,filename="./run_log/cityscape_drn38_4gLTE_baseline2.log",filemode="w")
...
def test(eval_data_loader, model, num_classes,
    ...
    search_tables=pd.read_hdf("./youtube_drn22_log/table_youtube_drn22_baseline2.hdf5",key="df",mode="r")
    ...
    for iter, (image, label, name) in enumerate(eval_data_loader):
        ...
        if prev_gray is not None :
            if  iter_fps<(upload_fps)<(iter_fps+(1/17.0)) or iter%17==0 or fps==16:
                ...
                """1. 使用jpeg压缩并重建 """
                #传播量化表1
                q1 = qtables[qp-1].reshape(1, 3, 1, 8, 8) # 第一帧使用jpeg（Q=85）量化表
                ...
                #save_encode_images(image.clone().cpu().data.numpy(),name,output_dir='/media/xxd/新加卷/Projects/DRN/save-fps/jpeg_image/B={0}-fps={1}'.format(B,fps))
                ...
                save_output_images(pred, name, output_dir='/media/xxd/新加卷/Projects/DRN/run_log/pred_img/cityscape_drn38_4gLTE/baseline2/')
                # next_pred = load_label('/media/xxd/新加卷/Projects/DRN/save-fps-drn22/baseline2/fps={0}/pred_img/B={1}-fps={2}'.format(int(fps),B,fps) + '/' + name[0].split("/")[-1])
                pred = load_label('/media/xxd/新加卷/Projects/DRN/run_log/pred_img/cityscape_drn38_4gLTE/baseline2/' + '/' + name[0].split("/")[-1])
                acc_jpeg = accuracy_np(pred, label[0].cpu().data.numpy())
